...
`src/`, so existing source-relative include spellings remain valid.  The stable
package line starts at `1.0.0` / `SOVERSION 1`: source and protocol
compatibility are checked, but cross-toolchain C++ ABI compatibility is not
promised.
