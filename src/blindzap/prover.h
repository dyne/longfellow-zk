...
template <size_t Keys>
inline bool BlindzapVerifyKeys(const BlindzapEnvelopeV1& e) { std::vector<std::array<uint8_t,20>> programs; if(!BlindzapDistinctPrograms(e.statement,&programs) || programs.size()!=Keys)return false; QuadCircuit<BlindzapField> q(p256k1_base); auto c=BlindzapBuildCircuit<Keys>(&q); if(!BlindzapProofIdentityMatches(e.proof,BlindzapExpectedCircuitDigest<Keys>()))return false; const size_t block=BlindzapBlock(*c,q); using CF=CrtConvolutionFactory<CRT256<BlindzapField>,BlindzapField>; using RS=ReedSolomonFactory<BlindzapField,CF>; CF cf(p256k1_base); RS rs(cf,p256k1_base); ZkProof<BlindzapField> p(*c,4,128,block); ReadBuffer rb(e.proof.bytes.data(),e.proof.bytes.size()); if(!p.read(rb,p256k1_base) || rb.remaining()!=0)return false; Dense<BlindzapField> pub(1,c->npub_in); { DenseFiller<BlindzapField> f(pub); f.push_back(p256k1_base.one()); BitPluckerEncoder<BlindzapField,8> enc(p256k1_base); for(const auto& program:programs)for(auto x:program)f.push_back(enc.encode(x)); } const auto seed=BlindzapTranscriptSeed(e.statement,e.proof); Transcript t(seed.data(),seed.size()); ZkVerifier<BlindzapField,RS> v(*c,rs,4,128,block,p256k1_base); v.recv_commitment(p,t); return v.verify(p,pub,t); }
