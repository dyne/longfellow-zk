...
def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("--cpp", required=True); parser.add_argument("--rust", required=True); parser.add_argument("--commit", required=True); parser.add_argument("--subset", choices=("all", "transcript", "field", "coding", "curve", "merkle"), default="all")
    ...
    keys = set(range(1, 7)) if args.subset == "transcript" else set(range(10, 16)) if args.subset == "field" else set(range(20, 26)) if args.subset == "coding" else set(range(30, 36)) if args.subset == "curve" else set(range(40, 43)) if args.subset == "merkle" else set(left)
