...
// The standalone harness intentionally has no subprocess/death-test facility.
// longfellow-zk exposes recoverable value_position/value_length APIs for the
// formerly fatal invalid-type paths, so the applicable non-death metadata
// expectations below exercise those APIs instead.
...
TEST(HostDecoderTest, Coverage) {
  // 1. Test position() for UNSIGNED
  {
    ...
    { size_t value_position = 0; assert(root.value_position(&value_position)); assert(value_position == 0); }
  }
  ...
  // 2. Test position() for TAG
  {
    ...
    size_t pos = 0;
    ASSERT_TRUE(root.decode(bytes.data(), bytes.size(), pos, 0));
    { size_t value_position = 0; assert(root.value_position(&value_position)); assert(value_position == 4); }  // Tagged text payload position
  }
  ...
  // 5. Test length() for large UNSIGNED
  {
    ...
    size_t pos = 0;
    ASSERT_TRUE(root.decode(bytes.data(), bytes.size(), pos, 0));
    { size_t value_length = 0; assert(root.value_length(&value_length)); assert(value_length == 5); }
  }
...
int main() {
  proofs::HostDecoderTest_DecodeHost();
  proofs::HostDecoderTest_Lookup();
  proofs::HostDecoderTest_Coverage();
  return 0;
}
